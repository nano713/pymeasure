...
class CS165MUM(Instrument):
    ...
    def set_parameters(self):
        with Image_Parameters() as image_processes:
...
if "__main__" == __name__:
    with TLCameraSDK() as sdk:
        ...
        with CS165MUM("27886") as set_exposure: 
            print(set_exposure.name)
            exposure = set_exposure.set_parameters()
            camera.issue_software_trigger()

        with ImageAcquisitionThread(camera) as image_acquisition: 
            print("Image acquisition started")
            print("Image acquisition finished")

        with Image_Parameters(camera) as image_save:
            image_save.save_image(image_acquisition.last_image, "saved_image.tif")
